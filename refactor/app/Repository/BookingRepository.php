...
//            $jobs['total'] = $totaljobs;
    ...
    public function store($user, $data)
    {
        ...
        if (!isset($data['from_language_id']) || !isset($data['duration'])) {
            $response['status'] = 'fail';
            $response['message'] = "Du måste fylla in alla fält";
            $response['field_name'] = !isset($data['from_language_id']) ? "from_language_id" : "duration";
            return $response;
        }
        ...
        if ($data['immediate'] == 'no') {
            if (empty($data['due_date']) || empty($data['due_time']) ||
                ...
                $response['status'] = 'fail';
                $response['message'] = "Du måste fylla in alla fält";
                $response['field_name'] = empty($data['due_date']) ? "due_date" : (empty($data['due_time']) ? "due_time" : "customer_phone_type");
                return $response;
            }
        } else {
        ...
        $data['gender'] = $data['certified'] = null;
        foreach ($data['job_for'] as $jobFor) {
            if (isset($jobForMappings[$jobFor])) {
                $value = $jobForMappings[$jobFor];
                if ($jobFor == 'male' || $jobFor == 'female') {
                    $data['gender'] = $value;
                } elseif ($jobFor == 'normal' || $jobFor == 'certified' ||
                    $jobFor == 'certified_in_law' || $jobFor == 'certified_in_helth') {
        ...
        if ($user->userMeta->consumer_type == 'rwsconsumer') {
            $data['job_type'] = 'rws';
        } elseif ($user->userMeta->consumer_type == 'ngo') {
            $data['job_type'] = 'unpaid';
        } elseif ($user->userMeta->consumer_type == 'paid') {
            $data['job_type'] = 'paid';
        }
    ...
    public function storeJobEmail($data)
    {
        ...
        $sendData = ['user' => $job->user, 'job' => $job];
        $this->sendEmail($email, $name, $subject, 'emails.job-created', $sendData);
    ...
    $this->sendSessionEmail($job, $postData, 'faktura');
    $this->sendSessionEmail($job, $postData, 'lön');
    ...
    private function sendSessionEmail($job, $postData, $forText)
    {
        $user = $job->user()->first();
        $email = $job->user_email ?: $user->email;
        $name = $user->name;
        $subject = 'Information om avslutad tolkning för bokningsnummer # ' . $job->id;
        $sessionTime = implode(' ', explode(':', $job->session_time));
        $data = ['user' => $user, 'job' => $job, 'session_time' => $sessionTime, 'for_text' => $forText];
    ...
    {
        $completeddate = now(); // Using Carbon for the current datetime
        ...
        $interval = $diff->format('%h:%i:%s');
        $job = $job_detail;
        $job->end_at = now(); // Using Carbon for the current datetime
        $job->status = 'completed';
        $job->session_time = $interval;
        ...
        // Using dependency injection instead of instantiating AppMailer directly
        app('AppMailer')->send($email, $name, $subject, 'emails.session-ended', $data);
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        $jobs = collect($job_ids)->reject(function ($job) use ($user_id) {
        ...
        foreach ($users as $user) {
            ...
            foreach ($jobs as $potentialJob) {
                if ($job->id == $potentialJob->id) {
                    $job_for_translator = Job::assignedToPaticularTranslator($user->id, $potentialJob->id);
                    if ($job_for_translator == 'SpecificJob') {
                        $job_checker = Job::checkParticularJob($user->id, $potentialJob);
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        } elseif ($job->customer_physical_type == 'no' && $job->customer_phone_type == 'yes' || $job->customer_physical_type == 'yes' && $job->customer_phone_type == 'yes') {
